...
namespace GodotSharp.Utilities.SourceGenerators
{
    ...
    public class AutoloadGenerator : ISourceGenerator
    {
            ...
            INamedTypeSymbol[] godotClassesWithAttr = context
                ...
                .SelectMany(tree =>
                    tree.GetRoot().DescendantNodes()
                        ...
                        .Where(x => x.symbol.GetAttributes().Any(a => a.AttributeClass?.IsAutoloadAttribute() ?? false))
                        .Select(x => x.symbol))
        ...
        {
            AttributeData? attribute = symbol.GetAttributes().FirstOrDefault(x => x.AttributeClass?.IsAutoloadAttribute() ?? false);
            if (attribute == null)
                return;

            string nameSpace = symbol.ContainingNamespace.FullQualifiedNameOmitGlobal();
            string className = attribute.ConstructorArguments[0].Value?.ToString()!;

            List<string> allUsings = new List<string> { symbol.ContainingNamespace.FullQualifiedNameOmitGlobal(), "Godot" };
            ...
            foreach (IMethodSymbol method in publicMethods)
            {
                ...
                for (int i = 0; i < parameters.Length; i++)
                {
                    ...
                    {
                        string defaultVal = parameter.ExplicitDefaultValue != null ? parameter.ExplicitDefaultValue.ToString() : "null";
                        if (parameter.ExplicitDefaultValue is bool)
                            defaultVal = defaultVal.ToLower();
                    
                        finalClass.Append($" = {defaultVal}");
                    }
